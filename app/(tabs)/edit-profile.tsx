...
export default function EditProfile() {
	...
	useLayoutEffect(() => {
		async function fetchUser() {
			try {
				const data = await api.users.findById(
					authState?.userId,
					authState?.token ?? 4,
				);
		...
		<SafeAreaView className="bg-primary h-full pb-4">
			<ScrollView>
				<View className="w-full justify-center min-h-[95vh]">
					...
					>
						<Text className="text-3xl text-brand font-inter-semibold text-center mt-20">
							Edit Profile
						</Text>
						...
						<Controller
							...
							rules={{
								...
								maxLength: {
									value: 255,
									message:
										"Your profile description is too long. Please shorten it to 255 characters or less.",
								},
							}}
							render={({ field: { onChange, value } }) => (
								<FormField
