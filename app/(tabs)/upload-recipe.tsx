...
export default function UploadRecipe() {
	...
	const handleIngredientInputChange = (text) => {
		...
		if (text.trim() !== "") {
			...
			setIsValidIngredient(
				filtered.some(
					(ingredient) => ingredient.name.toLowerCase() === text.toLowerCase(),
				),
			); // Valida si el ingrediente es exacto
		} else {
	...
	const addIngredient = () => {
		if (
			isValidIngredient &&
			!ingredients.some((ing) => ing.name === ingredientInput)
		) {
			const newIngredient = {
				name: ingredientInput,
				quantity: ingredientQuantities[ingredientInput] || 1,
			};
			...
			Alert.alert(
				"Error",
				"El ingrediente debe seleccionarse de la lista de autocompletado y no debe estar ya en la lista.",
			);
		}
	};
	...
	const handleTagInputChange = (text) => {
		...
		if (text.trim() !== "") {
			...
			setIsValidTag(
				filtered.some((tag) => tag.name.toLowerCase() === text.toLowerCase()),
			); // Valida si el tag es exacto
		} else {
			setFilteredTags([]);
	...
	const addTag = () => {
		if (isValidTag && !tags.some((tag) => tag.name === tagInput)) {
			...
			Alert.alert(
				"Error",
				"El tag debe seleccionarse de la lista de autocompletado y no debe estar ya en la lista.",
			);
		}
	};
	...
	const submitRecipe = async () => {
		...
		if (image) {
			...
			const imageResponse = await api.recipes.uploadImage(
				image,
				authState.token,
			);
		...
		const recipeResponse = await api.recipes.uploadRecipe(
			recipe,
			authState.token,
		);
		...
		if (recipeResponse.isSuccess) {
			...
			for (let i = 0; i < ingredients.length; i++) {
				const ingredient = {
					id_ingrediente: 0,
					nombre: ingredients[i].name,
					medida: "al gusto",
					dias_vencimiento: 0,
				};
				const ingredientResponse = await api.recipes.uploadIngredient(
					ingredient,
					authState.token,
				);
				ingredientsWithQuantities.push({
					ingredientId: ingredientResponse.data.ingrediente.id_ingrediente,
					quantity: ingredients[i].quantity,
				});
			}

			const relationResponse = await api.recipes.relateIngredientsToRecipe(
				recipeId,
			...
			if (relationResponse.isSuccess) {
				...
				const tagRelationResponse = await api.recipes.relateTagsToRecipe(
					recipeId,
					tagIds,
					authState.token,
				);
				...
				if (tagRelationResponse.isSuccess) {
					Alert.alert(
						"Éxito",
						"Receta, ingredientes y tags subidos exitosamente",
					);
				} else {
					Alert.alert(
						"Error",
						"Hubo un problema al relacionar los tags con la receta",
					);
				}
			} else {
				Alert.alert(
					"Error",
					"Hubo un problema al relacionar los ingredientes con la receta",
				);
			}
		} else {
		...
		>
			<StyledScrollView className="bg-white h-full p-4">
				<StyledView className="items-center mt-12 mb-6">
					...
					/>
					<StyledText className="text-3xl font-bold text-yellow-600 mb-4">
						Subir Receta
					</StyledText>
				</StyledView>
					...
					{filteredIngredients.length > 0 && (
						<StyledView className="border border-gray-300 rounded-lg max-h-40 overflow-hidden">
							{filteredIngredients.map((item) => (
								...
								>
									<StyledText className="p-2 bg-gray-200 border-b border-gray-300">
										{item.name}
									</StyledText>
								</TouchableOpacity>
							))}
				...
				<StyledTouchableOpacity
					onPress={pickImage}
					className="bg-yellow-600 p-4 mb-4 rounded-lg"
				>
					<StyledText className="text-white text-center">
						Seleccionar Foto de la Receta
					</StyledText>
				</StyledTouchableOpacity>
